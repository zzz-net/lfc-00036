...
describe('批量重算——差异明细四类 change_type', () => {
  ...
  it('【回归复现】命名参数缺失时 bulkInsert 必须抛 Missing named parameter——复现修复前 new_anomaly_id 缺失故障', () => {
    seedBasicData(db);
    const taskId = seedTask(db);
    ...
    const brokenAdded = {
      task_id: taskId, change_type: 'added',
      student_id: 'S00001', student_name: '张三', grade: '高三', class_name: '1班',
      anomaly_type: 'late', anomaly_date: '2026-06-15',
      new_description: '上午迟到 7 分钟', new_status: 'pending',
    };
    expect(() => db.prepare(BULK_INSERT_SQL).run(brokenAdded)).toThrow(/Missing named parameter/);

    const brokenRemoved = {
      task_id: taskId, change_type: 'removed',
      student_id: 'S00002', student_name: '李四', grade: '高三', class_name: '1班',
      anomaly_type: 'absent', anomaly_date: '2026-06-16',
      old_description: '上午缺勤', old_status: 'pending', old_anomaly_id: 42,
    };
    expect(() => db.prepare(BULK_INSERT_SQL).run(brokenRemoved)).toThrow(/Missing named parameter/);

    const brokenKept = {
      task_id: taskId, change_type: 'kept',
      student_id: 'S00003', student_name: '王五', grade: '高二', class_name: '2班',
      anomaly_type: 'late', anomaly_date: '2026-06-17',
      old_description: '迟到', new_description: '迟到',
      old_status: 'pending', new_status: 'pending', old_anomaly_id: 99,
    };
    expect(() => db.prepare(BULK_INSERT_SQL).run(brokenKept)).toThrow(/Missing named parameter/);
  });

  it('【回归修复】完整 14 字段（含显式 undefined）四类 change_type 应全部成功落库且 summary 正确', () => {
    seedBasicData(db);
    const taskId = seedTask(db);
    const BULK_INSERT_SQL = `
      INSERT INTO recalc_details (task_id, change_type, student_id, student_name, grade, class_name, anomaly_type, anomaly_date, old_description, new_description, old_status, new_status, old_anomaly_id, new_anomaly_id)
      VALUES (@task_id, @change_type, @student_id, @student_name, @grade, @class_name, @anomaly_type, @anomaly_date, @old_description, @new_description, @old_status, @new_status, @old_anomaly_id, @new_anomaly_id)
    `;
    const stmt = db.prepare(BULK_INSERT_SQL);

    const validAdded = {
      task_id: taskId, change_type: 'added',
      student_id: 'S00001', student_name: '张三', grade: '高三', class_name: '1班',
      anomaly_type: 'late', anomaly_date: '2026-06-15',
      old_description: undefined, new_description: '上午迟到 7 分钟',
      old_status: undefined, new_status: 'pending',
      old_anomaly_id: undefined, new_anomaly_id: undefined,
    };
    const validRemoved = {
      task_id: taskId, change_type: 'removed',
      student_id: 'S00002', student_name: '李四', grade: '高三', class_name: '1班',
      anomaly_type: 'absent', anomaly_date: '2026-06-16',
      old_description: '上午缺勤', new_description: undefined,
      old_status: 'pending', new_status: undefined,
      old_anomaly_id: 42, new_anomaly_id: undefined,
    };
    const validKept = {
      task_id: taskId, change_type: 'kept',
      student_id: 'S00003', student_name: '王五', grade: '高二', class_name: '2班',
      anomaly_type: 'late', anomaly_date: '2026-06-17',
      old_description: '上午迟到 3 分钟', new_description: '上午迟到 3 分钟',
      old_status: 'pending', new_status: 'pending',
      old_anomaly_id: 99, new_anomaly_id: undefined,
    };
    const validKeptModified = {
      task_id: taskId, change_type: 'kept_modified',
      student_id: 'S00001', student_name: '张三', grade: '高三', class_name: '1班',
      anomaly_type: 'late', anomaly_date: '2026-06-18',
      old_description: '上午迟到 5 分钟', new_description: '上午迟到 10 分钟',
      old_status: 'pending', new_status: 'pending',
      old_anomaly_id: 101, new_anomaly_id: undefined,
    };

    expect(() => stmt.run(validAdded)).not.toThrow();
    expect(() => stmt.run(validRemoved)).not.toThrow();
    expect(() => stmt.run(validKept)).not.toThrow();
    expect(() => stmt.run(validKeptModified)).not.toThrow();

    const total = (db.prepare('SELECT COUNT(*) as c FROM recalc_details WHERE task_id=?').get(taskId) as any).c;
    expect(total).toBe(4);

    const added = db.prepare('SELECT * FROM recalc_details WHERE task_id=? AND change_type=?').get(taskId, 'added') as any;
    expect(added.old_description).toBeNull();
    expect(added.old_status).toBeNull();
    expect(added.old_anomaly_id).toBeNull();
    expect(added.new_anomaly_id).toBeNull();
    expect(added.new_description).toBe('上午迟到 7 分钟');

    const removed = db.prepare('SELECT * FROM recalc_details WHERE task_id=? AND change_type=?').get(taskId, 'removed') as any;
    expect(removed.new_description).toBeNull();
    expect(removed.new_status).toBeNull();
    expect(removed.new_anomaly_id).toBeNull();
    expect(removed.old_anomaly_id).toBe(42);

    const counts = db.prepare('SELECT change_type, COUNT(*) as c FROM recalc_details WHERE task_id=? GROUP BY change_type').all(taskId) as any[];
    const map: Record<string, number> = {};
    for (const r of counts) map[r.change_type] = r.c;
    expect(map.added).toBe(1);
    expect(map.removed).toBe(1);
    expect(map.kept).toBe(1);
    expect(map.kept_modified).toBe(1);

    const byGrade = db.prepare('SELECT grade, change_type, COUNT(*) as cnt FROM recalc_details WHERE task_id=? AND grade IS NOT NULL GROUP BY grade, change_type').all(taskId) as any[];
    expect(byGrade.length).toBeGreaterThan(0);
    const gaoSan = byGrade.find((r: any) => r.grade === '高三');
    expect(gaoSan).toBeTruthy();
  });
});
...
describe('批量重算——失败链路与非法输入防护', () => {
  it('POST /tasks 输入校验：空日期应返回 400（等价模拟：空日期正则不匹配）', () => {
    const dateRe = /^\d{4}-\d{2}-\d{2}$/;
    expect(dateRe.test('')).toBe(false);
    expect(dateRe.test('2026/06/15')).toBe(false);
    expect(dateRe.test('2026-6-15')).toBe(false);
    expect(dateRe.test('abc')).toBe(false);
    expect(dateRe.test('2026-06-15')).toBe(true);
    expect(dateRe.test('2026-13-40')).toBe(true); // 正则层面匹配，真实 Date 校验拦截
  });

  it('POST /tasks 输入校验：start_date > end_date 应返回 400', () => {
    expect('2026-06-22' > '2026-06-16').toBe(true);
    expect('2026-06-16' > '2026-06-22').toBe(false);
    expect('2026-06-16' > '2026-06-16').toBe(false);
  });

  it('POST /tasks 输入校验：规则版本 id 不存在应返回 404（等价：ruleVersionRepo.getById 返回 undefined）', () => {
    seedBasicData(db);
    const count = (db.prepare('SELECT COUNT(*) as c FROM rule_versions').get() as any).c;
    expect(count).toBe(0);
    const v = db.prepare('SELECT * FROM rule_versions WHERE id = ?').get(9999);
    expect(v).toBeUndefined();
  });

  it('任务失败态：捕获异常后应把任务标记为 failed 并记录 error_message + operation_log', () => {
    seedBasicData(db);
    const rulesSnapshot = JSON.stringify([
      { grade: '高三', morning_start_time: '07:20', late_tolerance_minutes: 3, afternoon_start_time: '13:50', absent_window_minutes: 120 },
    ]);
    const taskId = db.prepare(`
      INSERT INTO recalc_tasks (rule_snapshot, start_date, end_date, status, operator, progress_percent, progress_message)
      VALUES (?, '2026-13-45', '2026-13-46', 'running', '回归测试', 70, '写入差异明细...')
    `).run(rulesSnapshot).lastInsertRowid as number;

    const simulatedError = 'Missing named parameter "new_anomaly_id"';
    db.prepare(`
      UPDATE recalc_tasks SET status = 'failed', error_message = ?, finished_at = CURRENT_TIMESTAMP, progress_message = '重算失败'
      WHERE id = ?
    `).run(simulatedError, taskId);
    db.prepare(`
      INSERT INTO operation_logs (action, operator, target_type, target_id, summary, detail_json)
      VALUES ('recalc_failed', '回归测试', 'recalc_task', ?, ?, ?)
    `).run(String(taskId), `重算失败：${simulatedError}`, JSON.stringify({ error: simulatedError, task_id: taskId }));

    const task = db.prepare('SELECT * FROM recalc_tasks WHERE id = ?').get(taskId) as any;
    expect(task.status).toBe('failed');
    expect(task.error_message).toBe(simulatedError);
    expect(task.error_message).toContain('new_anomaly_id');
    expect(task.finished_at).toBeTruthy();

    const logs = db.prepare('SELECT * FROM operation_logs WHERE target_id = ?').all(String(taskId)) as any[];
    expect(logs.length).toBeGreaterThan(0);
    const failedLog = logs.find((l: any) => l.action === 'recalc_failed');
    expect(failedLog).toBeTruthy();
    expect(failedLog.summary).toContain('new_anomaly_id');
    expect(failedLog.detail_json).toBeTruthy();
    const parsed = JSON.parse(failedLog.detail_json);
    expect(parsed.task_id).toBe(taskId);
  });

  it('任务失败态：已存在 running 任务时 acquireRecalcLock 返回 ok=false（等价：getRunningOrQueued 长度>0）', () => {
    seedBasicData(db);
    db.prepare(`
      INSERT INTO recalc_tasks (rule_snapshot, start_date, end_date, status, operator, progress_percent)
      VALUES ('[]', '2026-06-15', '2026-06-16', 'running', '测试员', 30)
    `).run();
    const rq = db.prepare("SELECT * FROM recalc_tasks WHERE status IN ('queued','running')").all() as any[];
    expect(rq.length).toBe(1);
    expect(rq[0].status).toBe('running');
  });

  it('cancel 失败态：非 queued/running 状态任务应返回 400（等价：已 completed 任务不可取消）', () => {
    seedBasicData(db);
    const taskId = db.prepare(`
      INSERT INTO recalc_tasks (rule_snapshot, start_date, end_date, status, operator, progress_percent)
      VALUES ('[]', '2026-06-15', '2026-06-16', 'completed', '测试员', 100)
    `).run().lastInsertRowid as number;
    const t = db.prepare('SELECT * FROM recalc_tasks WHERE id = ?').get(taskId) as any;
    expect(t.status).toBe('completed');
    expect(t.status === 'queued' || t.status === 'running').toBe(false);
  });
});
